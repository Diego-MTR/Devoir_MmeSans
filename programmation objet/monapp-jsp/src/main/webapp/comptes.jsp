...
<head>
    ...
    function showConfirm(action, onConfirm) {
        ...
        let color = "#e74c3c";
        if (action === "supprimer") {
            actionText = "Êtes-vous sûr de vouloir supprimer ce compte ?";
            color = "#e74c3c";
        } else if (action === "modifier") {
            actionText = "Êtes-vous sûr de vouloir modifier ce compte ?";
            color = "#f39c12";
        } else if (action === "créer") {
            actionText = "Êtes-vous sûr de vouloir créer ce compte ?";
            color = "#2e7dff";
        }
...
<body>
        ...
        <div class="content-box">
            ...
            %>
                <li>
                    ...
                    <span>
                        <% if ("admin".equals(userRole)) { %>
                        <a href="compte?action=edit&id=<%= c.getId() %>" class="btn" onclick="return confirmAction('modifier', 'compte?action=edit&id=<%= c.getId() %>')">✏️</a>
                        <a href="#" class="btn" onclick="return confirmAction('supprimer', 'compte?action=delete&id=<%= c.getId() %>')">🗑️</a>
                        <% } %>
                    </span>
                </li>
            <%
    ...
    %>
    <script>notify("Compte ajouté avec succès", "success");</script>
    <%
        } else if ("success_delete".equals(notif)) {
    %>
    <script>notify("Compte supprimé avec succès", "success");</script>
    <%
        } else if ("success_edit".equals(notif)) {
    %>
    <script>notify("Compte modifié avec succès", "success");</script>
    <%
        }
    %>
</body>
